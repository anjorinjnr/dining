...
class UserController extends BaseController {
  ...
  /**
   * 
   * @param \Neartutor\Repository\UserRepository $userRepository
   * @param \Neartutor\Services\UserService $userService
   */
  ...
  public function uploadPicture($user) {
	if (Input::file('file')->isValid()) {
	  if (!$this->userService->updatePicture(Input::file('file'), $user)) {
		return $this->json(array("status" => "error", "errors" => $this->userService->errors()));
	  } else {
		return $this->successResponse();
	  }
	} else {
	  return $this->json(["status" => "error", "errors" => ['Expects field of type file named filed']]);
	}
	return $this->successResponse();
  }
